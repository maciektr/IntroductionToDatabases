...
class ConferencesGenerator:
    def __init__(self, cl, conf_id=0, day_id=0, workshop_id=0, res_id=0, workshop_res_id=0):
        ...
        self.start_workshop_res = workshop_res_id
    ...
    def get_random_workshop(self, day_id, day):
        ...
        res += "\nINSERT INTO Workshops (workshop_id, conference_day_id, start_time, end_time, topic, price, number_of_seats) VALUES (" + str(
        ...
        for _ in range(numb_of_res):
            res += '\n'
            res += self.get_random_workshop_reservation(self.start_workshop_id, day, price)
    ...
    def get_random_conf_reservation(self, day_id, day):
        ...
        due_price = self.faker.date_between(start_date=res_date, end_date=day)
        adult_seats = self.rand.randint(0,
                                        len(self.clients_generator.get_participants_generator().get_participants_ids()))
        student_seats = self.rand.randint(0, len(
            self.clients_generator.get_participants_generator().get_participants_ids()) - adult_seats)
